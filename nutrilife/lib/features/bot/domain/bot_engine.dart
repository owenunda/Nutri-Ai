...
class BotEngine {
  ...
  // --- Gaze tracking -----------------------------------------------------
  // El target llega en coordenadas normalizadas del bot (unidades = radio de
  // la bola). Se acota a [_kMaxGaze] para que el ojo no se salga de la esfera
  // visible; el valor suavizado hacia el que se interpolan los ojos cada
  // sample() vive en (_gazeDx, _gazeDy). _lastSampleT permite calcular el dt
  // que necesita el easing exponencial.
  static const double _kMaxGaze = 0.40;
  static const double _kGazeSmoothing = 8.0; // 1/s, mayor = mas rapido
  double _gazeTargetDx = 0;
  double _gazeTargetDy = 0;
  double _gazeDx = 0;
  double _gazeDy = 0;
  double _lastSampleT = 0;
  ...
  /// Punto al que deben mirar los ojos, en espacio normalizado del bot
  /// (1.0 = radio de la bola). Se clamp-a magnitud a [_kMaxGaze]; el motor
  /// suaviza el valor aplicado hacia ese target en cada sample().
  void setGaze(double dx, double dy) {
    final m2 = dx * dx + dy * dy;
    final max2 = _kMaxGaze * _kMaxGaze;
    if (m2 > max2) {
      final k = _kMaxGaze / math.sqrt(m2);
      _gazeTargetDx = dx * k;
      _gazeTargetDy = dy * k;
    } else {
      _gazeTargetDx = dx;
  ...
  BotFrame sample(double t) {
    // El easing exponencial usa dt; si el reloj retrocede (replay de tests
    // con t no monotono) lo tratamos como dt=0 para no retroceder la mirada.
    final dt = t > _lastSampleT ? t - _lastSampleT : 0.0;
    _lastSampleT = t;
    final k = 1 - math.exp(-_kGazeSmoothing * dt);
    _gazeDx = lerpD(_gazeDx, _gazeTargetDx, k);
    _gazeDy = lerpD(_gazeDy, _gazeTargetDy, k);
